package cuie.module05.slimvaluedisplay;
...
public class SlimValueDisplay extends Region {
    ...
    private final DoubleProperty animatedValue = new SimpleDoubleProperty(); //AnimatedValue
    ...
    private final AnimationTimer timer = new AnimationTimer() {
        private long lastTimerCall; //Wann wurde das letzte mal etwas gemacht

        @Override
        public void handle(long now) {
            if (now > lastTimerCall + (getPulse().toMillis() * 1_000_000L)) { //Ist es wieder Zeit etwas zu tun?
                performPeriodicTask();
                lastTimerCall = now;
            }
        }
    ...
    private void setupValueChangedListeners() {
        ...
        //Wenn sich einer der 3 Werte ändert muss UI neu gezeichnet werden
        //valueProperty()   .addListener((observable, oldValue, newValue) -> updateUI()); --> updateUI auf animatedValue
        valueProperty()   .addListener((observable, oldValue, newValue) -> startAnimation());
        ...
        animatedValue.addListener((observable, oldValue, newValue) -> updateUI());

        //Wenn sich outOfRangeProperty ändert muss der Timer gestartet werden
    ...
    private void setupBindings() {
        ...
        valueLabel.textProperty().bind(animatedValue.asString(FORMAT));
    }
    ...
    private void startAnimation(){
        ...
        timeline.getKeyFrames().setAll(new KeyFrame(Duration.millis(800),
                                        new KeyValue(animatedValue, getValue(), Interpolator.EASE_BOTH))); //Endpunkt: Originalwert (nach 800 ms soll Originalwert erreicht werden)
        timeline.play();
    }

    private void updateUI() {
        bar.setVisible(true);
        bar.setLength(getAngle(getValue())); //Winkel aufgrund des aktuellen Werts
    }
