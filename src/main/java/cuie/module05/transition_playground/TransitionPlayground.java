 ...
 * @author Dieter Holz
 */
public class TransitionPlayground extends BorderPane {
    ...
    private void addEventHandlers() {
        rectangle.setOnMouseClicked(event -> {
            if (translateTransition == null) { //Wenn noch keine initialisiert ist
                translateTransition = new TranslateTransition(Duration.seconds(0.5), rectangle); //initialisieren
                //Setter
                translateTransition.setInterpolator(Interpolator.EASE_BOTH); //Geschwindigkeit/Beschleunigung der Anmiation
                translateTransition.setFromX(200); //Ausgangsposition der Animation
                translateTransition.setByX(300); //von Ausgangsposition um soviel in x-Richtung verschieben.
                translateTransition.setCycleCount(4); //Wie oft
                translateTransition.setAutoReverse(true); //vor und zurück
            }
            if (!translateTransition.getStatus().equals(Animation.Status.RUNNING)) { //bevor Animiation neu gestartet wird muss überprüft werden, ob sie schon läuft
                translateTransition.play(); //nur neu starten, wenn sie noch nicht läuft --> !
            }
        });
        ...
        text.setOnMouseClicked(event -> {
            if (rotateTransition == null) {
                rotateTransition = new RotateTransition(Duration.seconds(5), text);
