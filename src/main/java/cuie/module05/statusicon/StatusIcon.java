package cuie.module05.statusicon;
...
public class StatusIcon extends Region {
    public enum Status {
        ...
        CLOSED_HORIZONZAL(new Point2D(25, 40), new Point2D(75, 40), //Linie 1 (Startpunkt, Endpunkt)
                          new Point2D(25, 50), new Point2D(75, 50), //Linie 2 (Startpunkt, Endpunkt)
                          new Point2D(25, 60), new Point2D(75, 60)), //Linie 3 (Startpunkt, Endpunkt)
        ...
        //Zur Abfrage wo die Linien starten / enden
    ...
    // all timelines
    private final Timeline timeline = new Timeline(); //Zur Verwendung von Keyframes braucht man immer eine Timeline
    ...
    private void setupValueChangedListeners() {
        statusProperty().addListener((observable, oldValue, newValue) -> updateUI()); //UpdateUI aufrufen
    }
    ...
    private void updateUI(){
        //todo: was immer getan werden muss wenn sich der Status aendert
        if(timeline.getStatus().equals(Animation.Status.RUNNING)){
            //entweder laufende Animation laufen lassen: return;
            //oder Animation stoppen und neue starten: timeline.stop();
            timeline.stop();
        }
        Status stat = getStatus();

        KeyFrame kf = new KeyFrame(Duration.millis(500),
                                    new KeyValue(line1.startXProperty(), stat.getLine1Start().getX()),
                                    new KeyValue(line1.startXProperty(), stat.getLine1Start().getY()),
                                    new KeyValue(line1.endXProperty(), stat.getLine1End().getX()),
                                    new KeyValue(line1.endXProperty(), stat.getLine1End().getY()),

                                    new KeyValue(line2.startXProperty(), stat.getLine2Start().getX()),
                                    new KeyValue(line2.startXProperty(), stat.getLine2Start().getY()),
                                    new KeyValue(line2.endXProperty(), stat.getLine2End().getX()),
                                    new KeyValue(line2.endXProperty(), stat.getLine2End().getY()),

                                    new KeyValue(line3.startXProperty(), stat.getLine3Start().getX()),
                                    new KeyValue(line3.startXProperty(), stat.getLine3Start().getY()),
                                    new KeyValue(line3.endXProperty(), stat.getLine3End().getX()),
                                    new KeyValue(line3.endXProperty(), stat.getLine3End().getY()));
