...
public class NumberRangeControl extends Control {
    ...
    private static final CssMetaData<NumberRangeControl, Color> OUT_OF_RANGE_COLOR_META_DATA = FACTORY.createColorCssMetaData("-out-of-range-color", s -> s.outOfRangeColor);

    private final StyleableObjectProperty<Color> outOfRangeColor = new SimpleStyleableObjectProperty<>(OUT_OF_RANGE_COLOR_META_DATA, this, "outOfRangeColor");
    ...
    void setValueViaPercentage(double percentage) {
        boolean animated = isAnimated();
        setAnimated(false);

        setValue(percentageToValue(percentage, getMinValue(), getMaxValue()));
